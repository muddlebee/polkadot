...
impl<T: Config> ParachainsCache<T> {
	...
	fn ensure_loaded(&mut self) -> &mut Vec<ParaId> {
		if self.parachains.is_none() {
			self.parachains = Some(Parachains::<T>::get());
		}
		self.parachains
			.as_mut()
			.expect("ensured above that the parachains list is Some; qed")
	}

	/// Adds the given para id to the list.
	pub fn add(&mut self, id: ParaId) {
		let parachains = self.ensure_loaded();
